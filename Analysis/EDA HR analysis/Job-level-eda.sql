...
SELECT MIN(`Last hire date`) AS first_record
FROM divdata;	

-- Find all records within FY 2011
SELECT * FROM divdata WHERE YEAR(`Last hire date`) = 2011
...
WITH t1 AS (SELECT YEAR(`Last hire date`) AS yr,COUNT(emp_id) AS total_emp FROM divdata
GROUP BY 1 ORDER BY 1)
SELECT yr, total_emp, SUM(total_emp) OVER (ORDER BY yr) AS running_tot FROM t1;

-- Find the average number of employees hired over the entire dataset
-- Use common table expressions (CTE) & WINDOW( ) function to do it
WITH t1 AS (SELECT YEAR(`Last hire date`) AS yr, COUNT(emp_id) AS total_emp FROM divdata GROUP BY 1 ORDER BY 1)
SELECT COUNT(yr) AS total_yrs, ROUND((SUM(total_emp) / COUNT(yr)),2) AS avg_hire_num FROM t1;
...
SELECT  YEAR(`Last hire date`) AS yr, COUNT(CASE WHEN Gender = "M" THEN emp_id END) AS male_hires,
COUNT(CASE WHEN Gender = "F" THEN emp_id END) AS female_hires FROM divdata GROUP BY 1 ORDER BY 1;

-- STEP 2: Get running total & pct of male & female hires
-- CREATE TEMPORARY TABLE hiring_trend_pct_and_running_tot_by_yr_gender
SELECT yr, male_hires, 
...
FROM hiring_trend_by_yr_gender GROUP BY 1;
...
alter table divdata rename column `FY20 leaver?` to fy20leaver;
SELECT * FROM divdata WHERE fy20leaver!= "FY20";
-- -------------------------------------------------------------------
# # # # # # # # # # # # # # # JOB LEVEL # # # # # # # # # # # # # # #
-- -------------------------------------------------------------------

-- Find which job level is uneven
alter table divdata rename column `Job Level group for PRA` to fy20_jl_group_pra;
alter table divdata rename column `Job Level group PRA status` to fy20_jl_group_pra_status;

SELECT DISTINCT fy20_jl_group_pra
FROM divdata
WHERE fy20_jl_group_pra_status = "Uneven - Men benefit";

-- Get total count as well (incl FY20 leavers)

SELECT DISTINCT fy20_jl_group_pra, COUNT(emp_id) AS total_emp_count
FROM divdata
WHERE fy20_jl_group_pra_status = "Uneven - Men benefit"
GROUP BY 1;
...
SELECT DISTINCT `Fy20JLGroupPra`, COUNT(emp_id) AS total_emp_count
FROM divdata as de LEFT JOIN fact_table as f ON f.EmpID = de.emp_id
WHERE `Fy20JLGroupPraStatus` = "Uneven - Men benefit"
GROUP BY 1;	

-- Get the same results for job level & dept but excluding FY20 leavers
SELECT DISTINCT `Fy20JLGroupPra`, COUNT(emp_id) AS total_emp_count
FROM divdata as de LEFT JOIN fact_table as f ON f.EmpID = de.emp_id
WHERE `Fy20JLGroupPraStatus` = "Uneven - Men benefit" GROUP BY 1; 	 			
...
SELECT `Region group: nationality 1`AS region, gender, `Age group`,
COUNT(CASE WHEN gender = "M" THEN emp_id ELSE emp_id END) AS tot_emp
FROM divdata as de LEFT JOIN fact_table as f ON f.EmpID = de.emp_id 
LEFT JOIN `Region group: nationality 1`as drc ON drc.country_id = de.region_country_id 
WHERE fy20_jl_group_pra_status = "Uneven - Men benefit"
...
SELECT `Job Level after FY21 promotions` AS AgeGroup,
COUNT(CASE WHEN Gender = "M" THEN emp_id ELSE emp_id END) AS tot_emp
FROM divdata as de LEFT JOIN fact_table as f ON f.EmpID = de.emp_id 
WHERE `fy20_jl_group_pra_status` = "Even" GROUP BY 1, 2, 3 ORDER BY 1 DESC, 4 DESC;

-- STEP 2: Display grand total column next to tot_emp
-- CREATE TEMPORARY TABLE jl_multi_tot_by_gender_age
SELECT job_level, gender, age_group, tot_emp,
...
FROM divdata GROUP BY 1 ORDER BY 1 	DESC;
...
(SELECT COUNT(emp_id) FROM divdata WHERE fy20_jl_aft_promo = '3 - Senior Manager') AS grand_total_hire
...
SELECT COUNT(emp_id) AS tot_active_senior_managers FROM divdata
WHERE fy20_jl_aft_promo = "3 - Senior Manager"AND fy20leaver!= "Yes";
	
-- How many of the 3 - Senior managers were new hire?
alter table divdata rename column `New hire FY20?` to fy20_new_hire;
SELECT 
COUNT(emp_id) AS total_emp,
COUNT(CASE WHEN fy20_new_hire = "No" THEN emp_id END) AS old_emp,
COUNT(CASE WHEN fy20_new_hire = "Yes" THEN emp_id END) AS fy20_new_hire FROM divdata
WHERE fy20_jl_aft_promo = "3 - Senior Manager" AND fy20leaver != "Yes";
...
(SELECT COUNT(emp_id) FROM divdata WHERE fy20_jl_aft_promo = "3 - Senior Manager" AND fy20_new_hire = "Yes"))*100,0), "%") AS pct_of_total_hires
FROM divdata WHERE fy20_jl_aft_promo = "3 - Senior Manager" AND fy20leaver != "Yes" GROUP BY 1;

-- Were there any promotions from 4 - Manager → 3 Senior Manager for 2021?
SELECT 
(SELECT COUNT(emp_id) FROM divdata WHERE fy20_jl_aft_promo = "3 - Senior Manager" AND fy20leaver!= "Yes") AS fy20_tot_sen_manager,
COUNT(emp_id) AS fy21_tot_sen_manager,
COUNT(emp_id) - (SELECT COUNT(emp_id) FROM divdata WHERE fy20_jl_aft_promo = "3 - Senior Manager" AND fy20leaver != "Yes") AS num_promoted,
CONCAT(ROUND(((COUNT(emp_id) - (SELECT COUNT(emp_id) FROM divdata WHERE fy20_jl_aft_promo = "3 - Senior Manager" AND fy20leaver != "Yes")) / 
(SELECT COUNT(emp_id) FROM divdata WHERE `Job Level after FY21 promotions` = "3 - Senior Manager" AND fy20leaver != "Yes"))*100,2), "%") AS emp_pct_increase
FROM divdata WHERE `Job Level after FY21 promotions` = "3 - Senior Manager" AND fy20leaver != "Yes";
