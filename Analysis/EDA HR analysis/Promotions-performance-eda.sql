...
alter table divdata rename column `Promotion in FY20?` to fy20_promo;
SELECT `Last hire date`AS wrking_yrs,
...
COUNT(CASE WHEN fy20_07_01_dept = "Strategy" THEN emp_id END) AS strat_promo FROM divdata
WHERE fy20_promo = "Yes" GROUP BY 1 ORDER BY 1;
...
alter table divdata rename column `Promotion in FY21?` to fy21_promo;
SELECT 
COUNT(emp_id) AS fy20_total_emp,
COUNT(CASE WHEN fy20_promo = "Yes" THEN emp_id END) AS fy20_promo_num,
(SELECT COUNT(emp_id) FROM divdata WHERE `Leaver FY`!= "FY20") AS fy21_total_emp,
COUNT(CASE WHEN fy21_promo = "Yes" THEN emp_id END) AS fy21_promo_num FROM divdata;

-- STEP 2a: Split out fy20 data from table
-- CREATE TEMPORARY TABLE fy20_total_emp_and_num_promoted
SELECT fy20_total_emp AS tot_emp, fy20_promo_num AS num_promoted
FROM emp_id_and_fy21_promo_fy20_promo;
...
SELECT* FROM fy20_total_emp_and_num_promoted UNION SELECT* FROM fy21_total_emp_and_num_promoted;

-- STEP 4: Create new year column for indication
-- CREATE TEMPORARY TABLE tot_emp_and_promoted_by_yr
SELECT CASE  WHEN tot_emp = 500 THEN 2020 ELSE 2021 END AS yr,
tot_emp, num_promoted
FROM tot_emp_and_promo_unpivot;

-- STEP 5: Compare promotion percentage
SELECT*,CONCAT(ROUND((num_promoted / tot_emp)*100,2), "%") AS pct_promoted
...
SELECT  `Last Department in FY20` AS dept, COUNT(emp_id) AS fy20_total_promo FROM divdata
WHERE fy20_promo = "Yes" GROUP BY 1;

-- STEP 2: Get promotions by dept for FY21
-- CREATE TEMPORARY TABLE fy21_promotions_by_dept
SELECT `Last Department in FY20` AS dept,COUNT(emp_id) AS fy21_total_promo
FROM divdata WHERE fy21_promo = "Yes" GROUP BY 1;

-- STEP 3: Join STEP 1 & STEP 2 tables & compare the pct increase for the departments
SELECT fy20.dept, fy20_total_promo, fy21_total_promo,
CONCAT(ROUND(((fy21_total_promo - fy20_total_promo) / fy20_total_promo)*100,2),"%") AS pct_increase
FROM fy20_promotions_by_dept fy20 LEFT JOIN fy21_promotions_by_dept fy21
...
WITH t2 AS (SELECT gender, COUNT(emp_id) AS total_promo
  FROM divdata
  WHERE fy20_jl_aft_promo = "3 - Senior Manager"
  AND fy20_last_dept = "Internal Services"
  AND fy20_promo = "Yes"
  GROUP BY 1
  ORDER BY 1 DESC)
SELECT t1.gender,t1.total_emp, t2.total_promo
FROM fy20_is_sn_mngr_tot_emp_gender t1
LEFT JOIN t2 ON t2.gender = t1.gender
...
WITH t2 AS (SELECT gender,COUNT(emp_id) AS total_promo FROM divdata
WHERE fy21_jl_aft_promo = "3 - Senior Manager" AND fy20_last_dept = "Internal Services" AND fy21_promo = "Yes"GROUP BY 1
ORDER BY 1 DESC) SELECT t1.gender,t1.total_emp, t2.total_promo
...
SELECT CASE WHEN gender = "M" THEN 2020 ELSE 2020 END AS yr,
(SELECT COUNT(emp_id) FROM divdata WHERE fy20_jl_aft_promo = "3 - Senior Manager" AND fy20_last_dept = "Internal Services") AS tot_emp,
...
CASE WHEN gender = "F" AND total_promo IS NULL THEN 0 WHEN gender = "F" THEN total_promo ELSE 0 END AS fy20_f_promo
...
(SELECT COUNT(emp_id) FROM divdata WHERE fy21_jl_aft_promo = "3 - Senior Manager" AND fy20_last_dept = "Internal Services") AS tot_emp,
...
WHEN gender = "F" THEN total_promo ELSE 0 END AS fy21_f_promo
...
SELECT yr, tot_emp, SUM(fy20_males) AS males,
...
FROM fy20_is_sn_mngr_tot_emp_and_promo_by_gender UNION
SELECT yr, tot_emp, SUM(fy21_males) AS males, SUM(fy21_females) AS females, SUM(fy21_m_promo + fy21_f_promo) AS tot_promo,
SUM(fy21_m_promo) AS m_promo,
SUM(fy21_f_promo) AS f_promo FROM fy21_is_sn_mngr_tot_emp_and_promo_by_gender								
...
WITH t2 AS (SELECT gender,COUNT(emp_id) AS total_promo FROM dim_emp WHERE fy20_jl_aft_promo = "3 - Senior Manager"
AND fy20_last_dept = "Sales & Marketing" AND fy20_promo = "Yes" GROUP BY 1 ORDER BY 1 DESC)
SELECT t1.gender,t1.total_emp, t2.total_promo
...
WITH t2 AS (SELECT gender,COUNT(emp_id) AS total_promo FROM divdata
WHERE fy21_jl_aft_promo = "3 - Senior Manager"
AND fy20_last_dept = "Sales & Marketing" AND fy21_promo = "Yes" GROUP BY 1 ORDER BY 1 DESC)
SELECT t1.gender,t1.total_emp, t2.total_promo
...
(SELECT COUNT(emp_id) FROM divdata WHERE fy20_jl_aft_promo = "3 - Senior Manager" AND fy20_last_dept = "Sales & Marketing") AS tot_emp,
CASE WHEN gender = "M" THEN total_emp ELSE 0 END AS fy20_males,
CASE WHEN gender = "F" THEN total_emp ELSE 0 END AS fy20_females,
CASE WHEN gender = "M" THEN total_promo ELSE 0 END AS fy20_m_promo,
CASE WHEN gender = "F" AND total_promo IS NULL THEN 0 
WHEN gender = "F" THEN total_promo ELSE 0 END AS fy20_f_promo
...
FROM (SELECT *,SUM(emp_count) OVER (PARTITION BY rating_jump) AS rating_jump_group_total FROM (SELECT 
rating_jump,gender,COUNT(rating_jump) AS emp_count FROM fy21_non_promo_emp_w_perf_improv GROUP BY 1, 2 ORDER BY 1, 2
) subq1)subq2
...
WITH t1 AS (
SELECT yrs_since_last_hire AS non_promoted_wrking_yrs,
COUNT(yrs_since_last_hire) AS non_promoted_tot_count FROM dim_emp
WHERE fy21_promo_base_group = "Yes"AND fy21_promo = "No" AND fy20_new_hire = "No"GROUP BY 1)
SELECT 
de.yrs_since_last_hire AS tot_yrs_in_comp, COUNT(de.yrs_since_last_hire) AS promoted_tot_count,
t1.non_promoted_tot_count
FROM dim_emp de LEFT JOIN t1 ON de.yrs_since_last_hire = t1.non_promoted_wrking_yrs
WHERE fy21_promo_base_group = "Yes" AND fy21_promo = "Yes"
...
FROM (WITH t1 AS (SELECT yrs_since_last_hire AS tot_wrk_yrs, gender,
COUNT(yrs_since_last_hire) AS promoted_tot_count
FROM dim_emp WHERE fy21_promo_base_group = "Yes" AND fy21_promo = "Yes"
GROUP BY 1, 2 ORDER BY 1)
SELECT de.yrs_since_last_hire AS tot_yrs_in_comp, de.gender, t1.promoted_tot_count,
COUNT(de.yrs_since_last_hire) AS non_promoted_tot_count
FROM dim_emp de LEFT JOIN t1 ON de.yrs_since_last_hire = t1.tot_wrk_yrs AND de.gender = t1.gender WHERE fy21_promo_base_group = "Yes"
AND fy21_promo = "No" AND fy20_new_hire = "No" GROUP BY 1, 2 ORDER BY 1
) subq1

-- STEP 2a: Bring in avg performance of FY21 promoted employees within each category of working years (Excl. gender)
SELECT *,CASE WHEN avg_fy19_perf_rating = 0 THEN avg_fy20_perf_rating ELSE ((avg_fy20_perf_rating - avg_fy19_perf_rating)*-1) 
END AS tot_increase FROM (SELECT
de.yrs_since_last_hire AS tot_yrs_in_comp, promoted_tot_count,
ROUND(AVG(fy19_perf_rating),2) AS avg_fy19_perf_rating,
ROUND(AVG(fy20_perf_rating),2) AS avg_fy20_perf_rating
FROM dim_emp de JOIN tot_work_yrs_all_emp twyae ON twyae.tot_yrs_in_comp = de.yrs_since_last_hire
WHERE fy21_promo_base_group = "Yes" AND fy21_promo = "Yes" GROUP BY 1 ORDER BY 1
)subq1
...
FROM (SELECT *,SUM(tot_count) OVER (PARTITION BY job_level) AS jl_total FROM (SELECT fy20_jl_aft_promo AS job_level, gender,
COUNT(fy20_jl_aft_promo) AS tot_count-- COUNT(emp_id)
FROM dim_emp
WHERE fy20_last_dept = "Sales & Marketing" GROUP BY 1, 2 ORDER BY 1, 2)subq1
) subq2
