...
SELECT Gender,COUNT(Gender) AS gender_breakdown,
(SELECT COUNT(Gender) AS total_hires FROM divdata WHERE fy20_new_hire = "Yes") AS total_hires
FROM divdata
WHERE fy20_new_hire = "Yes"
GROUP BY 1;

-- STEP 2: Show the % of hires by gender
SELECT Gender, gender_breakdown AS total_hires, 
...
SELECT gender,COUNT(gender) AS gender_breakdown,(SELECT COUNT(gender) FROM dim_emp WHERE fy20_new_hire = "No") AS grand_total
FROM divdata
