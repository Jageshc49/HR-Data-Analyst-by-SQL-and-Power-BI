...
alter table divdata rename column `Age @01.07.2020` to fy20_jul_age;
select*from divdata;
SELECT
...
END AS age_group, `Last hire date` AS hire_date,  						-- Use hire date to be more specific with naming convention
CASE 									-- Use 'from date' to indicate the date this employee starts a new position (if any)
WHEN `Last hire date` < "2019-01-01" THEN "2019-01-01"
ELSE `Last hire date`
END AS from_date,
CASE 									-- Use 'end date' to indicate when this record will expire
WHEN YEAR(`Last hire date`) < 2020 THEN "2019-12-31"
ELSE "uh oh... re-check logic"
END AS end_date, `Years since last hire` - 1 AS yrs_since_last_hire,`fy20_last_dept`AS dept,
`fy20_promo` AS job_level, `FY19 Performance Rating` AS perf_rating,
CASE 		                                  -- fy20_jl_aft_promo AS next_yr_job_level,
WHEN YEAR(`Last hire date`) < 2020 THEN "N"     -- Use 'current_flag' as an indicator of whether this record is active or not
ELSE "Y"
END AS current_flag,
CASE 									-- Used to tell whether this employee was employed in the company within the specified year 
WHEN `Leaver FY` IS NOT NULL THEN "Y"
ELSE "N"
END AS employed_flag,
CASE 									-- Used to indicate whether someone left the company within a specified year
WHEN `Leaver FY` IS NOT NULL THEN "N"
END AS leaver_flag,
...
WHEN `In base group for turnover FY20` IS NOT NULL THEN "N"
...
END AS end_date, yrs_since_last_hire, contract_id, region_country_id, fy20_last_dept AS dept,
fy20_jl_aft_promo AS job_level, fy20_perf_rating AS perf_rating, fy21_promo AS jl_promo,
...
ELSE "Y"
END AS new_hire_flag, fy20_base_group_turnover AS turnover_base_group,
...
SELECT emp_id, gender, fy20_jul_age + 1 AS age, 				-- Have to subtract age by 1 because we're getting emp data from 1 year ago 
...
WHEN fy20_jul_age + 1 BETWEEN 50 AND 59 THEN "50 to 59"
WHEN fy20_jul_age + 1 BETWEEN 60 AND 69 THEN "60 to 69"
END AS age_group,last_hire_date AS hire_date, 
CASE 									-- Use 'from date' to indicate the date this employee starts a new position (if any)
...
END AS end_date,yrs_since_last_hire + 1 AS yrs_since_last_hire,  contract_id, region_country_id, fy20_last_dept AS dept,
...
SELECT (SELECT COUNT(emp_id) FROM dim_emp) - COUNT(emp_id) AS starting_fy21_employees FROM divdata WHERE fy20_leaver = "Yes"; 		-- FY21 has 453 employees
...
SELECT * FROM dim_emp_jl_group_pra
...
UNION
SELECT * FROM dim_emp_fy21) t1 LEFT JOIN fact_main t2 ON t2.emp_id = t1.emp_id
LEFT JOIN dim_emp_jl_group_pra_no_zero t3 ON t3.fy20_jl_group_pra = t2.fy20_jl_group_pra
LEFT JOIN dim_emp_jl_dept_group_pra_no_zero t4 ON t4.fy20_jl_dept_group_pra = t2.fy20_jl_dept_group_pra
...
SELECT emp_id, gender, age, age_group, hire_date, from_date, end_date, yrs_since_last_hire, 
...
FROM scd_id_status_null t1 LEFT JOIN dim_emp_jl_group_pra_no_zero t2 ON t2.fy20_jl_group_pra = t1.fy20_jl_group_pra
