...
alter table divdata rename column `Last Department in FY20` to fy20_last_dept;
SELECT fy20_last_dept, Gender,
COUNT(emp_id) AS tot_emp FROM divdata
...
SELECT fy20_last_dept, Gender,
COUNT(emp_id) AS tot_emp FROM divdata
WHERE `Leaver FY`= "FY20" GROUP BY 1, 2
ORDER BY 1, 2;
...
t1.tot_emp, 
CASE WHEN t2.tot_emp IS NULL THEN 0 ELSE t2.tot_emp END AS tot_leavers,
SUM(t1.tot_emp) OVER (PARTITION BY t1.fy20_last_dept) AS dept_gender_total
FROM fy20_tot_emp_by_gender_dept t1 LEFT JOIN fy20_tot_leavers_by_gender_dept t2 ON t2.fy20_last_dept = t1.fy20_last_dept
AND t2.gender = t1.gender)subq1;
 
 -- STEP 4: Get total count of males & female leavers
SELECT Gender, SUM(tot_emp) AS gender_total, SUM(tot_leavers) AS totl_leavers;																		
...
SELECT fy20_last_dept, gender, COUNT(emp_id) AS tot_emp
FROM dim_emp
WHERE fy20_jl_aft_promo = "3 - Senior Manager"AND leaver_fy = "FY20"
GROUP BY 1, 2

-- STEP 3: Combine STEP 1 & STEP 2 tables together
-- CREATE TEMPORAY TABLE fy20_tot_sn_mngr_tot_emp_andleavers
SELECT *FROM 
(SELECT t1.fy20_last_dept AS dept_name, t1.gender, t1.tot_emp, 
CASE
WHEN t2.tot_emp IS NULL THEN 0
ELSE t2.tot_emp
END AS tot_leavers
FROM fy20_tot_sn_mngr t1
LEFT JOIN fy20_tot_sn_mngr_leavers t2 ON t2.fy20_last_dept = t1.fy20_last_dept
